document.addEventListener('DOMContentLoaded', () => {
    ...
    const historyData = window.EventFlow ? window.EventFlow.getPlayerHistory() : [];
    ...
    function renderHistory(data) {
        ...
        if (data.length === 0) {
            container.innerHTML = `
                <div style="text-align: center; grid-column: 1/-1; padding: 60px 20px;">
                    <i class="bi bi-clipboard-x" style="font-size: 48px; color: #1e293b; margin-bottom: 15px; display: block;"></i>
                    <p style="color: #94a3b8; font-size: 16px;">No previous events found matching your search criteria.</p>
                </div>
            `;
            return;
        }
        ...
        data.forEach(event => {
            ...
            const badgesHtml = event.badges.map(b => `<span class="badge">${b}</span>`).join('');
